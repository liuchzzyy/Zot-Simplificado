...
2. 再点击工具栏中的 **双语** 开始翻译
3. 关闭阅读模式会自动结束双语、移除译文并取消尚未发出的翻译请求；重新进入阅读模式后需手动开启双语
...
插件会通过 GitHub Release 检查新版本。当前版本为 `1.2.1`，安装包名称为 `zot-simplificado-1.2.1.xpi`。
...
运行 `node --test tests/*.test.cjs` 检查阅读模式退出、异步切换和翻译队列取消。
